...
static void avl_nodo_destruir(AVL_Nodo* raiz,
  ...
  if (raiz != NULL) {
    ...
    avl_nodo_destruir(raiz->der,
      funcs);
    funcs->destK(raiz->key);
    funcs->destV(raiz->value);
    free(raiz);
  }
}
void avl_destruir(AVL arbol,FUNC funcs) {
  avl_nodo_destruir(arbol->raiz, funcs);
  free(arbol);
}

static void* avl_nodo_buscar(AVL_Nodo* raiz,
  void* dato,FUNC funcs) {
  if (raiz == NULL) {
    return NULL;
  } else if (funcs->compK(dato, raiz->key) == 0) {
    return funcs->cpyV(raiz->value);
  }
  else if (funcs->compK(dato, raiz->key) < 0) // dato < raiz->key
...
/**
 * avl_nodo_altura: Funcion interna
 * que retorna la altura del arbol.
...
/**
 * avl_nodo_max_altura_hijos: Funcion
 * interna que retorna la maxima altura de
 * los hijos.
 */
...
/**
 * avl_nodo_factor_balance: Funcion
 * interna que retorna el factor de balance de
 * un nodo.
 */
...
/**
 * avl_nodo_rotacion_simple_izq: Funcion
 * interna que realiza una rotacion simple
...
/**
 * avl_nodo_rotacion_simple_der: Funcion
 * interna que realiza una rotacion simple
...
/**
 * avl_nodo_crear: Funcion interna
 * que crea un nuevo nodo y lo retorna.
 * La altura de un nodo hoja es 0.
 */
static AVL_Nodo* avl_nodo_crear(void* key,
    void* value, FUNC funcs) {
  AVL_Nodo* nuevoNodo = malloc(sizeof(AVL_Nodo));
  assert(nuevoNodo != NULL);
  nuevoNodo->key = funcs->cpyK(key);
  nuevoNodo->value = funcs->cpyV(value);
...
static AVL_Nodo* avl_nodo_insertar(
    AVL_Nodo* raiz, AVL_Nodo* newNode,
    int* updated,unsigned long version,
    FUNC funcs) {
  if (raiz == NULL) { // insertamos el nuevo elemento
    *updated = 0;
    return newNode;
  }else if (funcs->compK(newNode->key, raiz->key) < 0) { // el dato debe ir en el subarbol izq
    raiz->izq = avl_nodo_insertar(raiz->izq,
     newNode, updated, version, funcs);
    // chequear balance
  ...
  else if (funcs->compK(newNode->key, raiz->key) > 0) { // el dato debe ir en el subarbol der
    ...
    *updated = 1;
    ...
    funcs->destK(newNode->key);
    free(newNode);
    return raiz;
}
int avl_insertar(AVL arbol, void* key,
  void *value, int *updated,
  unsigned long version, FUNC funcs) {
  AVL_Nodo* newNode = avl_nodo_crear(key, value, funcs);
  if(newNode != NULL) {
    *updated = 0;
...
/**
 * avl_validar: Retorna 1 si el arbol
 * cumple la propiedad de los arboles AVL,
 * y 0 en caso contrario.
 * avl_nodo_validar_abb: Funcion
 * interna que retorna 1 si el arbol cumple la
 * propiedad de los arboles BB,
 * y 0 en caso contrario.
 * avl_nodo_validar_altura_y_balance:
 * Funcion interna que retorna 1 si la
 * altura y el factor de balance de
 * los nodos son correctos, y 0 en caso
 * contrario.
 */
static int avl_nodo_validar_abb(AVL_Nodo* raiz,
  ...
  if (raiz == NULL)
    ...
    if (min != NULL && funcs->compK(raiz->key, min) <= 0)
      return 0;
    if (max != NULL && funcs->compK(max, raiz->key) <= 0)
      return 0;
    // y validar subarboles recursivamente
    return (avl_nodo_validar_abb(raiz->izq,
     min, raiz->key, funcs) &&
      avl_nodo_validar_abb(raiz->der,
       raiz->key, max, funcs));
  }
}
...
int avl_validar(AVL arbol, FUNC funcs) {
  return (
...
static AVL_Nodo* avl_nodo_eliminar(AVL_Nodo* raiz,
  ...
  if (raiz == NULL)
    return raiz;
  else if (funcs->compK(key, raiz->key) < 0) {
    ...
    // Si el nodo tiene ambos hijos
      ...
      funcs->destK(raiz->key);
      funcs->destV(raiz->value);
      raiz->key = funcs->cpyK(temp->key);
      raiz->value = funcs->cpyV(temp->value);
      raiz->der = avl_nodo_eliminar(raiz->der,
        key, funcs);
...
void avl_eliminar(AVL arbol, void* key, FUNC funcs) {
